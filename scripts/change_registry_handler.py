...
def main():
    ...
    assert metaregistry.get_registry(CRYPTO_REGISTRY_HANDLER_ID) == OLD_CRYPTO_REGISTRY_HANDLER
    num_pools = metaregistry.pool_count()
    num_registries = metaregistry.registry_length()
    num_pools_in_crypto_registry = CryptoRegistry.at(CRYPTO_REGISTRY).pool_count()
    
    call_data = metaregistry.update_registry_handler.encode_input(
        CRYPTO_REGISTRY_HANDLER_ID, CRYPTO_REGISTRY_HANDLER,
    )
    tx = proxy_admin.execute(metaregistry.address, call_data, txparams)
    
    assert metaregistry.pool_count() == num_pools + num_pools_in_crypto_registry
    assert metaregistry.registry_length() == num_registries
    assert metaregistry.get_gauge("0xD51a44d3FaE010294C616388b506AcdA1bfAAE46") != ZERO_ADDRESS

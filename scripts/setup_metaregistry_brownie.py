...
def main():
    ...
    for _, data in BASE_POOLS.items():
        ...
        tx = proxy_admin.execute(base_pool_registry, call_data, txparams)
    ...
    for _, pool in CRYPTO_REGISTRY_POOLS.items():
        ...
        # set up tx calldata for proxy admin:
        call_data = crypto_registry.add_pool.encode_input(
            pool["pool"],
            pool["lp_token"],
            pool["gauge"],
            pool["zap"],
        ...
        tx = proxy_admin.execute(crypto_registry, call_data, txparams)
        total_gas_used += tx.gas_used
    ...
    call_data = address_provider.add_new_id.encode_input(
        metaregistry.address,
        metaregistry_description,
    )
    tx = proxy_admin.execute(address_provider.address, call_data, txparams)
    total_gas_used += tx.gas_used
    ...
    sample_pool_list = metaregistry.find_pools_for_coins(
        "0x95aD61b0a150d79219dCF64E1E6Cc01f0B64C4cE",
        "0x853d955aCEf822Db058eb8505911ED77F175b99e",
    )
    RICH_CONSOLE.print_json(json.dumps(sample_pool_list, indent=4))

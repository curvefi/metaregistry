...
def crypto_registry_v1(CryptoRegistryV1, owner):
    registry = CryptoRegistryV1.deploy(address_provider().address, {"from": owner})
    raise
...
def address_provider_updated(crypto_registry_v1):
    add_provider = address_provider()
    tx = add_provider.set_address(5, crypto_registry_v1, {"from": owner})
    yield add_provider
...
def crypto_factory_handler(
    CryptoFactoryHandler, owner, metaregistry, address_provider_updated, crypto_registry_handler
):
    handler = CryptoFactoryHandler.deploy(
        metaregistry, address_provider_updated, 6, {"from": owner}
    )

...
def test_revert_unauthorised_add_registry_handler(metaregistry, alice):
    with brownie.reverts():
        tx = metaregistry.add_registry_handler(0, brownie.ZERO_ADDRESS, {"from": alice})
        assert tx.revert_msg == "dev: only owner"


def test_revert_unauthorised_update_registry_handler(metaregistry, alice):
    with brownie.reverts():

...
def test_add_pool(crypto_registry_v1, owner):
    ...
    assert crypto_registry_v1.get_base_pool(TRICRYPTO2_MAINNET) == ape.ZERO_ADDRESS
    ...
    assert (
        ...
        == [USDT, WBTC, WETH] + [ape.ZERO_ADDRESS] * 5
    )
...
def test_remove_pool(crypto_registry_v1, owner):
    ...
    crypto_registry_v1.add_pool(
        ...
        ape.ZERO_ADDRESS,  # _zap
        2,  # _n_coins
        "eursusd",  # _name
        ape.ZERO_ADDRESS,  # _base_pool
    ...
    assert crypto_registry_v1.get_zap(TRICRYPTO2_MAINNET) == ape.ZERO_ADDRESS
    assert crypto_registry_v1.get_lp_token(TRICRYPTO2_MAINNET) == ape.ZERO_ADDRESS
    assert crypto_registry_v1.get_pool_from_lp_token(CRV3CRYPTO_MAINNET) == ape.ZERO_ADDRESS
    ...
    assert crypto_registry_v1.get_gauges(TRICRYPTO2_MAINNET)[0][0] == ape.ZERO_ADDRESS
    assert crypto_registry_v1.get_gauges(TRICRYPTO2_MAINNET)[1][0] == 0

    assert crypto_registry_v1.get_coins(TRICRYPTO2_MAINNET) == [ape.ZERO_ADDRESS] * MAX_COINS
